...
export const createModelToggleComposable = <T extends string>(name: T) => {
  ...
  const useModelToggle = ({
    ...
    const hasUpdateHandler = computed(() => lodash.isFunction(props[updateEventKeyRaw]));
    ...
    const doHide = (event?: Event) => {
      ...
      if (lodash.isFunction(onHide)) {
        onHide(event);
      }
    };

    const show = (event?: Event) => {
      if (props.disabled === true || (lodash.isFunction(shouldProceed) && !shouldProceed())) return;
    ...
    const onChange = (val: boolean) => {
      if (!lodash.isBoolean(val)) return;
