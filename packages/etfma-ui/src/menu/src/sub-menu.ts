...
import type { CSSProperties, Component, ExtractPropTypes, VNodeArrayChildren } from 'vue';
...
export default defineComponent({
  ...
  setup(props, { slots, expose }) {
    ...
    const vPopper = ref<InstanceType<typeof EtfmaTooltip> | null>(null);
    ...
    const handleMouseenter = (event: MouseEvent | FocusEvent, showTimeout = props.showTimeout) => {
      if (event.type === 'focus') {
        return;
      }
      if (
    ...
    return () => {
      const titleTag: VNodeArrayChildren = [
        ...
        h(
          ElIcon,
          {
            ...
            style: {
              transform: opened.value
                ? (props.expandCloseIcon && props.expandOpenIcon) ||
                  (props.collapseCloseIcon && props.collapseOpenIcon && rootMenu?.props.collapse)
                  ? 'none'
                  : 'rotateZ(180deg)'
                : 'none',
            },
          ...
          {
            default: () =>
              lodash.isString(subMenuTitleIcon.value)
