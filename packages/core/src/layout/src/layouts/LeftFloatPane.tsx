import { Focusable, focusTracker, observer } from '@elcplat/lowcode-core'
      ...
      onMounted(() => {
        ...
        focusing = focusTracker.create({
          range: (e) => {
            ...
            if (
              document.querySelector('.lc-workbench-body')?.contains(target)
            ) {
              return true
            }
            ...
            if (docks && docks?.length) {
              return docks.some((dock) =>
                (dock.getDOMNode() as any)?.$el?.contains(target)
              )
            }
            return false
          },
