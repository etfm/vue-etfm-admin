...
import type { MenuRecordRaw, Recordable } from '@etfma/types';

export function getAllParentPath<T = Recordable<any>>(treeData: T[], path: string) {
  const menuList = findPath(treeData, (n) => n.path === path) as MenuRecordRaw[];
 ...
 * 路由过滤
 */
export function getMenu(menus: AppRouteRecordRaw[]) {
  // const { t } = useI18n();
...
// export function configureDynamicParamsMenu(menu: MenuRecordRaw, params: RouteParams) {
