...
export class PluginManager implements IPluginManager {
  ...
  constructor(contextApiAssembler: IPluginContextApiAssembler) {
    ...
    this.eventBus = createModuleEventBus('plugin');
  }
  ...
  async init(pluginPreference?: IPluginPreference) {
    ...
    const asyncPlugin = sequence.map((pluginName) => this.pluginsMap.get(pluginName)!.init());

    try {
      await Promise.all(asyncPlugin);

      this.setNotify('success');
    } catch (e) {
      logger.error(`Failed to init plugin, it maybe affect those plugins which depend on this.`);
      logger.error(e);
