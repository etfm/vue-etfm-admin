...
export const setStyle = (
  ...
  if (isObject(styleName)) {
    entriesOf(styleName).forEach(([prop, value]) => setStyle(element, prop as any, value as any));
  } else {
...
export const removeStyle = (element: HTMLElement, style: CSSProperties | keyof CSSProperties) => {
  ...
  if (isObject(style)) {
    keysOf(style).forEach((prop) => removeStyle(element, prop as any));
  } else {
...
export function addUnit(value?: string | number, defaultUnit = 'px') {
  ...
  loggerWarning(SCOPE, 'binding value must be a string or number');
}
